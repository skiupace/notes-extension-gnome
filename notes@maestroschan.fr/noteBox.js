...
export const NoteBox = class NoteBox {
    ...
    _buildNote() {
        ...
        if (this._autoFocus) {
            // TODO dynamically update this setting
            this.noteEntry.connect('enter-event', this._getKeyFocus.bind(this));
            this.noteEntry.connect(
                'leave-event',
                this._leaveKeyFocus.bind(this)
            );
        } else {
            this.noteEntry.connect(
                'button-press-event',
                this._getKeyFocus.bind(this)
            );
    ...
    _buildHeaderbar() {
        ...
        this.moveBox.connect(
            'button-press-event',
            this._onMovePress.bind(this)
        );
        ...
        ctrlButton.actor.connect(
            'button-press-event',
            this._onResizePress.bind(this)
        );
        ctrlButton.actor.connect(
            'motion-event',
            this._onResizeMotion.bind(this)
        );
        ctrlButton.actor.connect(
            'button-release-event',
            this._onRelease.bind(this)
    ...
    fixState() {
        let outX =
            this._x < 0 ||
            this._x > Main.layoutManager.primaryMonitor.width - 20;
        let outY =
            this._y < 0 ||
            this._y > Main.layoutManager.primaryMonitor.height - 20;
        if (outX || outY) {
        ...
        if (Number.isNaN(this.actor.width)) {
            this.actor.width = 250;
        }
        if (Number.isNaN(this.actor.height)) {
            this.actor.height = 200;
        }
        if (Number.isNaN(this._fontSize)) {
            this._fontSize = 10;
        }
        this._saveState();
    }
    ...
    _resizeTo(event_x, event_y) {
        ...
        let newHeight = Math.abs(
            this._y + this.actor.height - event_y + (this.grabY - this._y)
        );
    ...
    _loadText() {
        let file2 = GLib.build_filenamev([
            this._dataPath,
            this.id.toString() + '_text',
        ]);
        ...
        let file = Gio.file_new_for_path(
            this._dataPath + '/' + this.id.toString() + '_text'
        );
    ...
    _loadState() {
        let fname = GLib.build_filenamev([
            this._dataPath,
            this.id.toString() + '_state',
        ]);
